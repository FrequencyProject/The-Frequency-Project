...
class UnifiedVivicSession:
    ...
    def execute_baseline_calibration(self, sweep_duration_seconds: float = 120.0, sample_rate_hz: float = 100.0):
        ...
        print(f"[INIT] Launching mandatory {sweep_duration_seconds}-second ambient calibration sweep...")
        ...
        if "MOCK" in self.adapter.daemon.port.upper() or "PORT" in self.adapter.daemon.port.upper():
            print(" -> Simulation environment detected: Throttling calibration window to 2.0 seconds.")
    ...
    def execute_live_cycle(self, steps: int = 5):
        ...
        for step in range(1, steps + 1):
            ...
            # P1 OPTIMIZATION: train_step() now automatically handles data extraction,
            # calibration alignment, and forward/backward optimization execution natively 
            # inside a single hot-path call. It returns both the scalar loss value AND the 
            # already computed latent vector, eliminating the redundant second inference pass completely!
            loss_val, latent_vector = self.engine.train_step(
                ambient_means=self.ambient_means,
                ambient_stds=self.ambient_stds
            )
            
            # If the engine encounters a buffer warm-up starvation frame, skip cleanly
            if loss_val < 0.0 or latent_vector is None:
                print(" -> Telemetry buffers warming up... stabilizing synchronization cadence registers.")
                time.sleep(0.016)
                continue
            
            # Execute the 3-Sigma vector divergence monitoring tracking using the reuse token
