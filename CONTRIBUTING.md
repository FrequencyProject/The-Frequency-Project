...
2.1 The Development Lifecycle Gateways
...
**Local Quality Validation:** Before logging a commit, execute our repository's static type checker and code formatter locally to prevent automation compilation breaks:  
bash  
...
python validate\_config.py

3. Use code with caution.

2.2 Pull Request Acceptance Criteria
...
* **Simulation Verification:** Any change to the core ingestion code must successfully pass all automated execution assertions inside the tests/ suite and run error-free inside prototype\_simulation.py with the standard mock validation metrics intact.  
